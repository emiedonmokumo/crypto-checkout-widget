...
const Page = () => {
    ...
    return (
        <main className="w-full min-h-screen flex items-center justify-center p-4 max-sm:p-2 m-0">
            <div className="w-full max-w-md min-h-dvh sm:min-h-auto p-5 max-sm:p-0 max-sm:border-none max-sm:shadow-none border border-gray-300 rounded-3xl shadow-lg bg-white mx-auto flex flex-col">
                {formStep == 1 && <CheckoutHeader activeTab={activeTab} onChange={setActiveTab} />}
                <div
                    key={activeTab}
                    className="mt-4 transition-all duration-300 ease-in-out transform opacity-0 translate-y-2 animate-fade-in"
                >
                    {renderTabContent()}
                </div>
            </div>
